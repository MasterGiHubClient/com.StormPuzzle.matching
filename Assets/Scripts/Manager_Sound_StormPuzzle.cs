...
public class Manager_Sound_StormPuzzle : MonoBehaviour
{
    [SerializeField] private AudioMixer mixer_StormPuzzle;
    ...
    public void SetVolumeMusic_StormPuzzle(float volume)
    {
        mixer_StormPuzzle.SetFloat("Music", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
    ...
    public void SetVolumeSFX_StormPuzzle(float volume)
    {
        mixer_StormPuzzle.SetFloat("Sound", volume <= 0.0001f ? -80f : Mathf.Log10(Mathf.Max(0.0001f, volume)) * 20);
